...
struct AgentCardExpandedSections: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            // Turns are added collapsed; the user expands the ones they want to read. (We used to
            // auto-expand the latest turn on every new turn, which made a streaming Security
            // Agent's turn list churn open messily.)
            if let callLog, callLog.lifetimeCount > 0 {
                LLMCallLogSection(log: callLog, expandedCallIDs: $expandedTurnIDs, expandsNewestCall: false)
            }
