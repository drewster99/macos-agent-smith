...
- **Severity is calibrated on FREQUENCY as well as gravity.** A level that fires on every call stops being a signal and becomes unfilterable noise. `approvedWithoutReview` is `.info` despite running unjudged (it fires on every call when review is off; it stays visible under its own kind) — **user-confirmed 2026-09-21 after the consequence below was surfaced, so don't "fix" it to `.warning`**; `reviewCancelled` is `.warning`, not `.error`, because a user pressing Stop must not paint the transcript red.
- **The floor is what lets the bottom pane's default be quiet.** `TranscriptViewConfig.conversation` ships with the `toolCalls` and `securityReviews` groups HIDDEN (audited 2026-09-20). The task-scope axis does not reach them — a worker's tool rows carry a `taskID`, but **Smith's do not**, so measured over one real session they were 25% `tool_request` + 23% `tool_output` + 25% `security_review` against **12% actual conversation**. Hiding them is safe only because the floor still surfaces failures and WARN/UNSAFE/unavailable verdicts: of 57 security rows, 53 were routine approvals and the 4 WARNs came through. Don't re-enable those groups to "make errors visible" — that is the floor's job. Known consequence, raised with the user and **accepted 2026-09-21**: `approvedWithoutReview` is `.info`, so with per-emitter review switched OFF those rows are hidden in this pane by default (Smith's carry no `taskID`, so the task pane does not show them either). The alternative — promoting it to `.warning` so the floor rescues it — was rejected because it fires on EVERY call and would be unfilterable. Unhide the Security reviews group if you need to watch them.
...
**Validation telemetry: `ValidationMetricsLedger` (2026-07-28).** One JSONL row per landed verdict, appended to the GLOBAL `validation_metrics.jsonl` by the coordinator immediately after `recordCriterionVerdicts` returns `.recorded` — the single producer of verdict records, so the mirror cannot drift; superseded rounds (which wrote nothing) mirror nothing. It exists because the task-local ledger is deliberately destroyed by failed-task retry, reopen, and criteria edits, which erases exactly the contentious runs an economics analysis needs. It is telemetry ONLY — nothing in validation reads it back. Rows are flat, ISO-8601-dated, schema-versioned (`v`), and carry full traceability: task id/title, `parentTaskID` (per-template rollups), criterion id/name/waivable/default-validator flag, verdict + capped detail, round + contract version, validator name/hash, and the judging `modelID`/`providerID`. Execution truth comes from **live telemetry** (`JudgmentTelemetryBox`, a `Mutex` fed by `runValidator`'s `onResponse` across attempts and dynamic items): LLM turns, evidence tool calls, input/output/cache-read tokens, wall-clock `durationMs`, `attempts`, and `firstAttemptError` — the previously invisible rescued-retry population. Token fields are nil when only the persisted transcript was available (counts then derive from the runner's own `responseLog` rendering — acceptable for telemetry, never for behavior). Error rows also carry a typed `errorKind` (and `firstAttemptErrorKind`) via `ValidationErrorKind.classify` — a table over error messages this codebase itself composes (turn_exhaustion / timeout / transport / …; unmatched → "other", degrading a histogram, never a run; extend the table when adding an error message at the source). The file is heterogeneous JSONL discriminated by `rowKind`: per-judgment rows plus one `ValidationRoundOutcomeRow` per decided round (completed / escalated / failed_no_progress / rejections_returned, with settled/rejected/errored counts and the convergence counter) — the task-level "how did the round end" that judgment rows cannot express. SINGLE SYNCHRONIZED WRITER, off the cooperative pool: every session's runtime shares `ValidationMetricsLedger.shared`, a final class whose one private SERIAL `DispatchQueue` does every encode+write; `append` is a synchronous fire-and-forget enqueue (deliberately NOT an actor — the validation path never suspends on file I/O, and the blocking write runs on a GCD utility thread). `flush()` exists for tests/shutdown only. Runtimes default to NO ledger — the app opts in at construction; tests that don't opt in structurally cannot write the real file (same philosophy as `PersistenceManager.init(testingRoot:)`).
...
**The shared config pool + UUID indirection was retired 2026-07-31 (user decision "retire the pool").** A role's model is now a `ModelAssignment { providerID, modelID }` stored per session (`SessionState.agentAssignments: [AgentRole: ModelAssignment]`), NOT a `ModelConfiguration.id` into a shared `LLMKitManager.configurations` pool. There is no clone-on-edit and no `ensureDedicatedConfig`. A role's runtime tuning (temperature, token ceilings, thinking, effort) is NOT a property of a shared config object — it lives entirely in the per-`(role, model)` override store (`SharedAppState.roleModelConfigOverrides`, keyed `"role/providerID/modelID"`, persisted to `role_model_config_overrides.json`) and is resolved FRESH against live `ModelInfo` at provider-build time (`ModelConfigurationOverride.resolved(against:name:)`, permissive/no-clamp, non-blocking out-of-range warnings). The model picker (`AgentModelSettingsSection`) writes `agentAssignments[role]` directly; editing an override calls `setRoleModelConfigOverride`, which fires `notifyModelAssignmentsChanged` so every RUNNING session rebuilds its providers (overrides are global; the per-session `agentAssignments.didSet → scheduleProviderRefresh` covers only the editing session's model swaps).

**When a rebuilt provider actually reaches a live agent depends on WHAT changed, and the two answers are deliberately different** (built 2026-09-20). `setProviders` classifies each role before merging:

- **A RETUNE — same `providerID` + `modelID`, different parameters** (temperature, effort, thinking, token caps) — is pushed to every LIVE agent of that role via `AgentActor.scheduleModelRetune`. The actor STAGES it in `pendingModelRetune` and applies it at the top of its next run-loop iteration (`applyPendingModelRetune`), the same boundary `pendingInjectedMessages` drains at. That boundary is load-bearing: it makes it impossible for one turn's LLM call, its tool results and its usage record to describe two different configurations, which is otherwise a live race because `LLMCallContext` is built AFTER every tool in the turn has run. Applied ahead of the iteration's `pruneHistoryIfNeeded`, so a changed context-window budget is honored by the prune preceding the next call rather than one turn late. It also rewrites `ToolContext.currentConfiguration`/`currentProviderType` (both now `var`), which stamp provenance on every channel message the agent posts. `learnedMaxOutputCeiling` is deliberately NOT cleared — it is a fact about the MODEL, which by definition did not change.
- **A MODEL or PROVIDER change never touches a live agent.** `scheduleModelRetune` refuses one and logs, fail-closed, even though `setProviders` already filtered it out. The stored conversation carries provider-shaped data (Anthropic thinking blocks, Gemini parts, Codex reasoning items) and tool-call ids minted in one provider's format; handing that to another backend ranges from silently wrong to a hard 400 (swapping TO Anthropic with thinking on, into a history with no thinking blocks, is unrecoverable without editing history). Brown picks a model change up at its next spawn; Smith keeps its model until the runtime COLD starts.
- **A role whose resolved `ModelConfiguration` is unchanged is not retuned at all.** Not an optimization: the app rebuilds every role's provider on every edit, and some providers mint per-instance state (the ChatGPT-subscription provider's `prompt_cache_key`, its prefix-cache routing hint), so retuning an untouched role would throw away a live agent's cache locality for a change that isn't there. The MERGE still happens for every role, because provider construction also reads capability/behavior overrides that `ModelConfiguration` equality cannot see.
...
- **`OrchestrationSettings`** is the complete/resolved value (Summarizer, five-point Retrieval, Validator, Security switches). **`OrchestrationSettingsOverride`** is the sparse all-optional delta; `nil` = inherit. `settings.applying(override)` is the ONE merge. Everything defaults ON in `.builtIn` except the retrieval matrix (per-point historical defaults; validator-review memory is OFF because injecting memories would make a verdict depend on the live corpus, which the validator audit hash doesn't cover).
- **Four layers, resolved left→right:** shipped (bundled `Resources/orchestration_defaults.json`, falls back to `.builtIn`) → downloaded (a complete replacement file, `orchestration_defaults_downloaded.json`; download feature deferred, slot wired) → app-wide user override (sparse, `orchestration_settings_override.json`, edited in the **Orchestration** Settings tab, Reset-to-defaults) → per-session override (`SessionState.orchestrationOverride`, edited from **Session ▸ Orchestration Overrides**). `SharedAppState.effectiveOrchestrationDefault` = system.applying(appOverride); `AppViewModel.resolvedOrchestrationSettings` = that.applying(session).
- **UI** reuses the model-override `OverrideTriStateRow` (Inherit / Force-on / Force-off); the tab and the per-session sheet share one form via `OrchestrationOverrideTarget` (app-wide vs session). The sheet shows the resolved value per row.
- **The engine reads a pushed snapshot: `OrchestrationRuntime.orchestrationSettings`** (`private(set)`, default `.builtIn`), set via `setOrchestrationSettings` and refreshed live — app-wide changes fan out to every running session (an observer registered at `start()`), a session's own override is pushed by `orchestrationOverride.didSet`. The runtime never loads files.
- **The guiding rule (user, 2026-08-01): NEVER change the architecture of WHAT gets called; add flags that change IN-CALL behavior.** Every subsystem is still invoked exactly as before; it reads its resolved switch first and either does its normal work or takes a disabled branch that reports it did so. OFF behaviors:
  - **Summarizer** — `summarizeAndEmbedTask` skips the write; `compactSmithContext` (+ `autoCompactSmithIfNeeded`) skip the LLM (the deterministic run-loop prune, `pruneNonBrownHistory`, remains the reduction path).
  - **Validators** — `performTaskValidation` completes the task via `completeValidatedTask(validationWasRun: false)` (banner `validationSkipped`, Smith told criteria were NOT judged), checked before the missing-model park.
  - **Scoping** — `scopeToolSetOnTaskStart` REPLACED the retired `preflightScopingEnabled` (deleted, not reused); read at the spawn gate + propagated to workers from `setOrchestrationSettings`. `setToolSecurity` is now `globalPolicy`-only.
...
- **Provider calls reach the inspector as `LLMCallEvent` (`completed` turn | `failed` attempt)**
  ...
  must emit here too, or its cost shows with no call behind it. The one exception is the Validator
  (`EvaluationRunner`): its record is the task verdict ledger, not a call log. A thrown call is a
...
**Decided 2026-09-22 (user):** `ModelProvider.liteLLMProviderName == nil` continues to mean that
the provider is unmapped. The literal mapping `LOCAL` is a distinct, intentional pseudo-provider
for endpoints whose models are outside LiteLLM. Agent Smith offers `LOCAL` in the provider-mapping
picker and renders it as a neutral local state: provider API facts and probe evidence still apply,
...
probeable capability means adding it to the deep battery. The Capabilities screen derives its
Standard/Deep label from the persisted profile's probed evidence; do not create a parallel marker
whose value can disagree with the probe record.
...
- **The Security Agent is an unconditional chokepoint: EVERY tool call from every agent routes through it.** There is no unevaluated branch left in `AgentActor` — `executeWithApproval` is the only way a tool runs, and it DENIES when no evaluator is configured rather than running the call unreviewed. The `mustEvaluate` predicate and the non-evaluated `directExecute` path were deleted rather than left as a constant-true test, so the invariant is structural instead of conditional. **`taskLifecycleTools` (`task_complete`, `request_help`, `reply_to_user`, `message_user`, `notify_brown`, `task_update`) route too** — they used to execute with "no approval" AND no `tool_request`, making them the quietest bypass in the system; they're now pre-cleared in the auto-approve table for the roles that emit them, so a worker can still say "done" or "blocked" without an LLM sitting in the path. The table **spells those names out rather than referencing `taskLifecycleTools`** — that set answers a sequencing question, and wiring it to the security carve-out would let anyone adding a lifecycle tool silently widen what skips review (fail-open). Duplication drifts the safe way: a name missing from the table just gets a real evaluation. The validator's `securityGate` likewise fails closed (it used to be `nil` — "reads execute directly" — when no Security Agent was configured). **The Security Agent cannot recurse — not because it is toolless, but because its tools never re-enter the gate.** It holds a read-only evidence pair: `file_read` always, plus `attach_file` when the runtime wired both attachment ingest and URL resolution (`SecurityAgentBehavior.toolNames`, `SecurityEvaluator.evalTools`) — it has to be able to look at the file a call is *about* before ruling on it. Those execute INLINE inside `SecurityEvaluator` (`executeSecurityAgentFileRead` / `executeSecurityAgentAttachFile`), never through `executeWithApproval`, so an evaluation structurally cannot spawn another one. Evidence rounds are bounded by `maxToolRounds` (16), after which the model is offered NO tools and must commit to a verdict. **Whether a tool call is reviewed was REINTRODUCED as a preference (2026-08-01, "Orchestration settings" below), deliberately reversing the earlier "not a preference" stance.** The Orchestration settings expose per-emitter review toggles (Agent Smith / Agent Brown / Validators); `SecurityEvaluator.evaluate` checks `reviewsToolCalls(callerRole)` FIRST and, when off, approves WITHOUT a verdict. The call STILL routes through the evaluator and is recorded + posted as "review disabled" (`SecurityDisposition.wasEvaluated == false`) — never as SAFE, and no control flow keys off the prose. The invariant preserved is VISIBILITY, not universal evaluation. The old `enablePerToolCheck` / `perCallApprovalEnabled` / `evaluatesOpenWorldToolsOnly` stay deleted — that was a different, INVISIBLE scheme (skipped calls vanished from the transcript); the new toggles keep every skipped call countable and in the transcript, which is the whole point.
- **What varies is verdict COST, not routing: `SecurityEvaluator.autoApprovedToolsByRole`.** A hardcoded `[AgentRole: Set<String>]` naming the calls approved without an LLM round-trip — still recorded, posted, and countable, so cheap never means invisible. **Fail-closed on both axes**: an unlisted role or unlisted tool gets a real evaluation, so new tools are judged until someone deliberately clears them. Smith's set covers messaging/task/scheduling/read-only inspection plus its stop controls (`terminate_agent`, `abort` — stopping is the risk-reducing direction, so a reviewer must never delay or veto one) and deliberately EXCLUDES its open-world tools and `attach_file` (its egress surface); Brown's holds only its task-lifecycle calls and read-only AppleScript discovery; the validator's is the read-only evidence set. Replacing this table with anything user-configurable reintroduces the hole it closed — the old scheme made an unreviewed tool an *invisible* one, absent from the transcript entirely.
- **The Security Agent provider is still REQUIRED regardless of the review toggles.** A missing provider fails `start()` outright (Smith's open-world tools are the softest exfiltration surface in the system, so starting Smith ungated is the one failure least likely to be noticed) and refuses `spawnBrown` — the toggles are runtime and can flip back on, so the evaluator must exist. What a review toggle changes is verdict COST, not the provider requirement: with it OFF, that emitter's calls run approved-but-unjudged (visibly, per above). A missing validator model still parks tasks instead of judging them (unless validation itself is disabled — then the task completes flagged unvalidated). No role falls back to another role's model — resist re-introducing that as a convenience.
...
- **A server that says no with its own error object is surfaced on the FIRST occurrence** (2026-09-23). oMLX answers HTTP 200 with `{"error": {"code": "prefill_memory_exceeded", …}}`; SwiftLLMKit ≥ 0.0.209 throws that as `LLMProviderError.responseFailed(code:message:)` instead of a parse failure, and the run loop posts it at once (still retried — it is transient) rather than hiding it behind the ≥5-consecutive gate. Memory refusals are recognized by the typed `LLMProviderError.serverMemoryExhaustion` (the server's `error.code`, never its prose): **three in a row** shrink the agent's context once per streak — the same reduction a context overflow uses — and post a user-addressed `.advisory` (`.warning`) with the server's reason and what to change (free server memory, the server's own memory ceiling, the role's "Max context tokens"). Any other outcome ends the streak. Add a `ServerMemoryExhaustionCode` case when another server spells this differently.
...
- **In the app target, only `@concurrent` means off-main. `async` doesn't, and neither does `nonisolated`.** The `.xcodeproj` sets `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` (everything is implicitly `@MainActor`) **and** `SWIFT_APPROACHABLE_CONCURRENCY = YES`, which enables `NonisolatedNonsendingByDefault` — so a `nonisolated async` function runs on its *caller's* actor, and the callers are views. `@Sendable` does not strip the default either. `AgentSmithPackage` sets neither, so in `AgentSmithKit` a nonisolated `async` function really does hop off. Consequences: an `await MainActor.run` written in app-target code is usually a **no-op**, and deleting it is required rather than optional — it asserts the surrounding work moved off main when it did not (this is what hid an O(m·n) LCS running on the main thread in `DiffView`). `@concurrent` implies `nonisolated` and must precede modifiers (`@concurrent nonisolated func`; the reverse is a parse error). Both settings are deliberate as of 2026-07-29; `scripts/actor_isolation_probe.swift` reproduces the whole table under each flag set, and ROADMAP.md ("Actor-isolation boundary") has the rejected alternatives. Before moving work off main for speed, **measure** — the same entry records a case where the shape looked identical and the work was 3% of the cost.
