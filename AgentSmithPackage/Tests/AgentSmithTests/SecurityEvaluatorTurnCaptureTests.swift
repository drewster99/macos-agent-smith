...
struct SecurityEvaluatorTurnCaptureTests {
    ...
    /// Replays `steps` in order and records every request it was sent.
    ...
    /// Collects call events from the evaluator's synchronous callback.
    ...
    func forcedVerdictRequestIsCaptured() async {
        var steps: [Step] = (1...16).map { .respond(fileReadCall($0)) }
