...
enum SmithBehavior {
    ...
    public static func systemPrompt(autoAdvanceEnabled: Bool = true) -> String {
        ...
        - Write `{{input_name}}` wherever a template's text needs the concrete value: the title, the description, any step, and any acceptance criterion (name, validation prompt, input enumerator prompt). Each placeholder is replaced with that run's value when the instance is created, so the worker and the validators read the real app name or path rather than the placeholder. WRITE THE PLACEHOLDERS — do not describe the inputs in prose and leave the text generic.
