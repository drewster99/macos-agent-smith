...
- [ ] **Template run-history in the top pane.** Selecting a template currently filters the top pane to `.task(template.id)` — which is empty (templates don't run). Instead show the template's run history (its instances) with drill-down into a run's transcript, per the original phase-4 spec. The sidebar/Library already shows the run list; this is the top-pane counterpart.
