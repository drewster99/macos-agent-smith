...
**Authoring checks landed in the store, at the point of mutation** — `updateDefinition` (against the prospective state, so an input and its references can be renamed in one call), `updateDescription`, `setSteps`, `applyStepAction`, `setAcceptanceCriteria`, `applyCriterionActions`, and a whole-task sweep in `setTemplateInputDefinitions` (the one edit that invalidates fields it doesn't touch). `TaskStore.addTask` has no way to refuse, so `CreateTaskTool` and `AppViewModel.createManualTask` check every field before creating. `updateDescription` and `setSteps` now return `String?` instead of `Bool`/`Void`. The task editor sheet runs the same check live as you type. Smith's prompt and the `create_task` / `edit_task` / `set_template_inputs` tool descriptions now tell it to write the placeholders instead of describing the inputs in prose.
