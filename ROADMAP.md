...
**Anthropic is model-gated with placement rules — all documentation-sourced, none of it measured yet.** Per Anthropic's docs, `{"role": "system"}` inside `messages` (as opposed to the top-level `system` parameter) is supported on Opus 5, Opus 4.8, Fable 5, and Mythos 5, and unsupported on Sonnet 5, which should return `role 'system' is not supported on this model`. **Sonnet 5 is therefore the highest-value target in the sweep, not a settled no** — it is the one model where the docs and the model metadata actively disagree (next paragraph), so a measured answer is worth the most there. Stating the doc claim as established fact would commit precisely the sin this entry exists to prevent. Where supported it must follow a `user` message (or an `assistant` message ending in server-tool use), must be either the last entry or be followed by an `assistant` turn, can never be `messages[0]`, and is text-only. Note this rules out the OpenAI-style "insert before the current user message" layout outright — on Anthropic the reminder goes *after* the user turn, as the last element.
...
Sonnet 5 deserves a note but not a legend. Anthropic's own documentation hedges on it — the prose says unsupported while noting that some internal model configuration says otherwise — but that is a remark in a doc, **not** an observable disagreement in any payload we can read, and earlier drafts of this entry overstated it as one. Treat it as the model whose documented answer is least load-bearing, so worth including in the first sweep; do not treat it as a known payload-vs-truth conflict, because we have no payload signal for this on any model.
...
**Where the code change lands.** Four touch points, mirroring how every other dimension was added: a new tri-state field on `ModelProfile` (`SwiftLLMKit/Evaluation/ModelProfile.swift`) carrying the usual `ProbeFinding` provenance; the contradiction test itself in `ModelProber` (`Evaluation/ModelProber.swift`); persistence/versioning in `ProbeRecordStore.swift` so `--reuse-store` and `--reuse-max-age-days` treat it like the rest; and driver wiring in the app-side `CapabilityEvalRunner.swift` (targets, seeding, the "probe only what isn't free" gate). Nothing structurally new — except the consumer, below.
