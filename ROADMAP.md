...
- [ ] **Closed-session transcript resolution (phase-3 sub-item).** The top pane binds to the CURRENT session's `TranscriptStore` filtered by `taskID`. Selecting a task whose origin session is closed (e.g. from the all-session browser or an archived task authored elsewhere) shows nothing. Build the read-only, file-backed store vended by `SessionManager` from a closed session's `channel_log.jsonl` (emits one `replaces:true`, no deltas), and route the top pane to it when `selectedTaskID`'s origin `sessionID` isn't the focused session.
- [x] **Template run-history in the top pane** (BUILT 2026-08-03). Selecting a template shows its runs (its instances, newest first) in the top pane; clicking a run drills into that run's transcript, with a "Run History" back affordance. `AppViewModel.selectedTemplateRunID` (transient) + `refreshTopTranscriptFilter` drive it; runs come from `childTasks` (this session's active + global archived). CAVEAT tied to the closed-session item below: drilling into a run whose ORIGIN session is closed shows an empty transcript until closed-session vending lands (the run still LISTS fine).
- [ ] **Template soft-delete / archive dual-dispatch.** `TaskStore.softDelete` / `archive` / `permanentlyDelete` don't handle a library-resident template (they return false). Today deletion is only via the Library row's "Remove from Library" (`removeTemplate`, hard). Decide whether a deleted template should be recoverable (file it under Recently Deleted with `isTemplate` preserved, restore back into the library) and dual-dispatch accordingly; also fixes the (near-unreachable) failed-template-creation rollback in `createManualTask`.
- [ ] **Promote/demote is not crash-ordered.** `commitTemplateFlip` does session-remove then library-upsert (promote) / library-remove then session-insert (demote) across an `await`, without the destination-durable-before-source-removal ordering the inactive-store `move` uses. A crash mid-flip could lose a just-promoted template from both files. Rare + user-initiated; mirror `move`'s durable ordering.
- [ ] **New template while the library is non-persistable.** If `template_library.json` is corrupt/unwritable (`templateLibraryPersistable == false`), `addTask` still upserts a new template into the in-memory library, which is never persisted → lost on quit. Route creation back to per-session when the library isn't persistable, or refuse.
- [ ] **`create_task(is_template:)` banner + retrieval.** `CreateTaskTool` re-reads the new template via `task(id:)` (per-session) for its `task_created` banner and `attachRelevantContext` → both no-op for a library-resident template, so the banner drops context chips and a retrieval embedding + corpus scan is wasted. Skip relevant-context for templates (it's instance-time work) or make `setRelevantContext` dual-dispatch.
- [ ] **Empty-group visibility.** `LibrarySectionView` hides the whole section when there are zero templates, so a group created while the library is empty isn't visible until a template lands in it. Minor; revisit if group-first authoring becomes a flow.
- [ ] **Downloaded orchestration defaults slot** (pre-existing, unrelated): the `orchestration_defaults_downloaded.json` layer is wired but the download feature itself is unbuilt.
